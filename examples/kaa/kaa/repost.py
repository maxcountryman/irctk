...
def format_reply(history):
    ...
    last_time = timesince(recent_time)

    if len(history) == 1:
        return '{0} linked that {1} ago.'.format(last_nick, last_time)

    hour_span = math.ceil((time.time() - history[-1][1]) / 3600)
    hour_span = '{0} hours'.format(hour_span if hour_span > 1 else 'hour')

    history_len = len(history)
    ordinal = ['once', 'twice', '{0} times'.format(history_len)]
    ordinal = ordinal[min(history_len, 3) - 1]
